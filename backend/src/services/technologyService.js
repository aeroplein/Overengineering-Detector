...
const technologyFieldsWithDuplicateCount = `
    ${technologyFields},
    COUNT(*) OVER (PARTITION BY LOWER(TRIM(name)), LOWER(TRIM(category)))::int AS duplicate_count
`;

export const getActiveTechnologies = async () => {
    const result = await pool.query(
        `SELECT DISTINCT ON (LOWER(TRIM(name)), LOWER(TRIM(category))) ${technologyFields}
         FROM technologies
         ...
         ORDER BY LOWER(TRIM(name)),
                  ...
                  (
                    ...
                    OR COALESCE(best_for, '') <> ''
                    OR COALESCE(risk_notes, '') <> ''
                    OR COALESCE(alternatives, '') <> ''
                    OR COALESCE(docs_url, '') <> ''
                  ) DESC,
                  id DESC`
    );
...
export const getAllTechnologiesForAdmin = async () => {
    const result = await pool.query(
        `SELECT DISTINCT ON (LOWER(TRIM(name)), LOWER(TRIM(category))) ${technologyFieldsWithDuplicateCount}
         FROM technologies
         ORDER BY LOWER(TRIM(name)),
                  LOWER(TRIM(category)),
